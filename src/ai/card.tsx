...
export function Card({
  ...
  const baseStyles =
    "bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-all duration-200 flex-1";
  const variantStyles = {
    default: "",
    outlined: "border-2",
    elevated: "shadow-lg",
  ...
  const handleActionClick = (action: string, e: React.MouseEvent) => {
    e.stopPropagation();
    onCallback?.({ type: "click", component: "card", data: { action, title } });
  };
