...
function TableComponent({ items, attributes, title, className }: TableProps) {
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden ${className}`}>
      {title && <h2 className="px-6 pt-6 pb-4 text-xl font-semibold text-gray-900 dark:text-gray-100">{title}</h2>}
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
              <th className="px-6 py-4 text-left font-semibold text-gray-900 dark:text-gray-100">Name</th>
              {attributes.map((attr) => (
                <th key={attr.key} className="px-6 py-4 text-left font-semibold text-gray-900 dark:text-gray-100">
                  {attr.label}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-200 dark:border-gray-600 transition-colors">
                ...
                {attributes.map((attr) => (
                  <td key={attr.key} className="px-6 py-4 text-gray-600 dark:text-gray-300">
